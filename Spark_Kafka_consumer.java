...
public class Spark_Kafka_consumer {
 ...
 /*     StreamingQuery ds = df
      ...
      StreamingQuery ds = df.selectExpr("CAST(key AS STRING)","CAST(value AS STRING)")
    		  .writeStream().outputMode("append").format("csv").option("path","hdfs://192.168.5.176:8020/user/root/abhilash/output")
   // 		  .option("topic", "test")
    		  .option("checkpointLocation","hdfs://192.168.5.176:8020/user/root/abhilash/output1")
    		  .start();      
      try {
		ds.awaitTermination();
